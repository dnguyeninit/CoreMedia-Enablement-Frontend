 ...
 * {@link #test} method returns true if this resolver is responsible to resolve references for the given parameters.
 * It returns false if {@link com.coremedia.livecontext.fragment.FragmentParameters#getExternalRef()} is null.
 *
 * <p>Note that the actual {@link #resolveExternalRef} method may still return null even if {@link #test} returned
...
public interface ExternalReferenceResolver extends Predicate<FragmentParameters> {
   ...
   * for the given parameters. In the latter case, method {@link #test} returns false when called with the given
   * parameters.
   *
